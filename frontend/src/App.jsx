...
const API_URL =
  `${BACKEND_URL}/api/v1/emails/analyze-jobs`;

const CASES_API_URL =
  `${BACKEND_URL}/api/v1/cases`;
...
function App() {
  const [isAuthenticated, setIsAuthenticated] = useState(
    () =>
      localStorage.getItem("satguard_authenticated") === "true" ||
      sessionStorage.getItem("satguard_authenticated") === "true"
  );
  ...
  const handleLogout = () => {
    localStorage.removeItem("satguard_authenticated");
    sessionStorage.removeItem("satguard_authenticated");
    localStorage.removeItem("satguard_user_email");
    ...
    try {
      ...
      while (!completed && attempts < maxAttempts) {
        ...
        if (statusData.status === "completed") {
          setData(
            normalizeForensicData(
              statusData.forensic_result
            )
          );
  ...
  return (
    <main className="app-shell">
      ...
      <section className="upload-panel">
        <div className="upload-heading">
          <div>
            ...
            <p>
              The original email is hashed before forensic
              analysis begins.
            </p>
          </div>
      ...
      <section className="two-column-layout">
        <OriginConfidence
          probableSource={
            data.relay_analysis.probable_source
          }
        />
      ...
      <InfrastructureMap
        ...
        receivedHops={
          data.relay_analysis.received_hops
        }
      />
      ...
      <footer className="footer-note">
        <p>
          SatGuard provides evidence-based infrastructure
          intelligence. IP data represents approximate network
          context, not a confirmed attacker identity or exact
          physical location.
        </p>
      </footer>
    </main>
